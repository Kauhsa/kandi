...
Optimoinnin ideana on vähentää mahdollisesti eri tietokoneella laskettavien *map*- ja *reduce*-laskentatehtävien välistä kommunikaatiota tekemällä välitulosten osittaista yhdistämistä jo *map*-laskentatehtävän sisällä. Kun *map*-laskentatehtävän välitulosten yhdistäminen tehdään samalla tietokoneella kuin itse *map*-laskentatehtävä, verkon yli *reduce*-laskentatehtäville lähetettävien välitulosten määrä vähenee. Kuva 3 näyttää esimerkin välitulosten yhdistämisestä.

*Combiner*-funktiona voidaan usein käyttää *reduce*-funktioksi määriteltyä funktiota, mutta sopiva funktio riippuu *map*-funktion luonteesta. Käytetään esimerkkinä *combiner*-funktioksi soveltumattomasta *reduce*-funktiosta funktiota, joka yhdistää välitulokset laskien kuinka monta *kissa*- tai *koira*-sanaa tarvitaan, jotta niiden lukumääräksi saataisiin sata.
...
PageRank on menetelmä, jolla voidaan järjestää Internet-sivuja tärkeysjärjestykseen niihin osoittavien linkkien perusteella [@pagerank]. Algoritmin ajatuksena on, että usein viitatut Internet-sivut ovat tärkeämpiä kuin vähemmän viitatut. Mitä tärkeämpi sivu on, sitä enemmän sen viittaukset nostavat viitattujen sivujen PageRank-arvoa. Google-hakukone sai alkunsa PageRank-algoritmista ja sen testaamisesta käytännössä [@pagerank]. Käytämme PageRank-menetelmää esimerkkinä hieman monimutkaisemmasta MapReduce-operaatiosta. 
