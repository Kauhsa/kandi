...
MapReduce-ohjelmointimalli ei rajoita miten syöte ladataan tai tulos tallennetaan. Syötteenä voidaan esimerkiksi käyttää joukkoa tiedostojärjestelmässä olevia tiedostoja, mutta ohjelmointimallin toteutus voi lisäksi mahdollistaa esimerkiksi tietokannan käytön syötteen lataamiseen ja tuloksen tallentamiseen [@mapreduce2, s. 74]. Jotta tiedostojen lataaminen ja tallentaminen ei muodostaisi pullonkaulaa MapReduce-laskentatehtäville, usein kuitenkin käytetään *hajautettua tiedostojärjestelmää*. Googlen esittelemä toteutus käyttää niin ikään Googlen kehittämää hajautettua tiedostojärjestelmää nimeltään *Google File System* (GFS) [@mapreduce], kun taas Apache Hadoop -projektiin kuuluu *Hadoop Distributed File System* (HDFS) -niminen hajautettu tiedostojärjestelmä [@hdfs]. Molemmissa hajautetuissa tiedostojärjestelmissä tiedostot jaetaan tiedostojärjestelmän tietoa säilövien tietokoneiden kesken.
